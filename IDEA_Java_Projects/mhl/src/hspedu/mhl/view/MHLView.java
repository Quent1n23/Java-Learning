package hspedu.mhl.view;

import hspedu.mhl.domain.*;
import hspedu.mhl.service.BillService;
import hspedu.mhl.service.DiningTableService;
import hspedu.mhl.service.EmployeeService;
import hspedu.mhl.service.MenuService;
import hspedu.mhl.utils.Utility;
...
public class MHLView {
    ...
    public void orderDiningTable() {
        ...
        if (key == 'Y') {//要预定
            //根据orderId 返回 对应的 DiningTable对象, 如果为null:该对象不存在
            ...
            //判断该餐桌的状态是否 "空"
            if (!("空".equals(diningTable.getState()))) {//不是 "空"状态
...
//        System.out.println("===============显示完毕===============");
    ...
    public void payBill() {
        ...
        if (!billService.hasPayBillByDiningTableId(diningTableId)) {
            System.out.println("===============该餐桌没有未结账的菜单===============");
            return;
        }
        System.out.print("结账方式(现金/支付宝/微信)回车表示退出: ");
        String payMode = Utility.readString(20, "");//如果回车,就是返回 "" 空串
        if("".equals(payMode)){
            System.out.println("===============取消结账===============");
            return;
        }
